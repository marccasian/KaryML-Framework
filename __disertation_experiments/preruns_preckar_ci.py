...
from __disertation_experiments.automation.get_best_initial_neurons_for_each_model import DIM, PRE_RUNS_FEATURES, \
    PRE_RUN_WEIGHTS, ds_entries, OVER_ID
...
def get_best_initial_input_neurons_file(classifier_type="SOM"):
    ...
    for i in feature_indexes:
        ...
        for ds_entry in ds_entries:
            ...
            runs.extend(db.get_similar_runs(a, over_id=OVER_ID))
        runs_preckar = [j.preckar for j in runs]
        stderr = np.std(runs_preckar) / np.sqrt(len(runs_preckar))
        if PRE_RUN_WEIGHTS[i]:
            print("Model {}; Features: {}, Weights: {}".format(i, PRE_RUNS_FEATURES[i],
                                                               ", ".join(str(ll) for ll in PRE_RUN_WEIGHTS[i])))
        else:
            print("Model {}; Features: {}".format(i, PRE_RUNS_FEATURES[i]))
...
if __name__ == '__main__':
    get_best_initial_input_neurons_file("FSOM")
